...
// GSAP " overwrite: 'auto' " fixes that problem. i will implement that later
...
class Canvas2D {
	...
	mount(id?: string, target?: HTMLElement) {
...
export class Gallery {
	...
	setPose(pose: Pose, force = false) {
		...
		// for remapping the layout i start from 0. it's easier to compute any offset you want.
	...
	draw() {
		...
		for (let i = 0; i < this.media.length; i++) {
			...
			// if ( !inRange( media.x, null, 0, this.viewport.width ) && !inRange( media.x + media.width, null, 0, this.viewport.width ) ) {
			// 	continue
			// }
...
function isCanvasSupported(contextID: string) {
	try {
		...
		return canvas && context ? true : false
	} catch (error) {
		return false
	}
}
...
function whichKey(event: KeyboardEvent) {
	...
	const Home = keyCode === 36 || code === 'Home' || key === 'Home'
	const End = keyCode === 35 || code === 'End' || key === 'End'
	...
	return {
		...
		Home,
		End,
		keyIsPressed: ArrowUp || ArrowDown || ArrowLeft || ArrowRight || Home || End,
	}
}
